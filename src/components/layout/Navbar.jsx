...
import { Sun, Moon, Menu, X, GraduationCap, User, LogOut, LogIn, UserPlus, ShieldCheck, FolderOpen } from 'lucide-react';
      ...
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center max-w-7xl">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {user && profile?.role !== 'admin' && (
            <Link to="/mis-proyectos" className={`font-medium transition-colors flex items-center gap-1.5 ${location.pathname === '/mis-proyectos' ? 'text-primary-600 dark:text-[#44bdff]' : 'text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-[#44bdff]'}`}>
        ...
        <div className="fixed inset-0 z-[60] bg-white dark:bg-gray-950 md:hidden flex flex-col animate-in fade-in duration-200">
          ...
          <div className="flex-1 overflow-y-auto pt-4 pb-10 px-8 flex flex-col">
            ...
            <div className="flex flex-col gap-8">
              ...
              {user && profile?.role !== 'admin' && (
                <Link to="/mis-proyectos" onClick={() => setIsOpen(false)} className="text-4xl font-bold text-primary-700 dark:text-[#44bdff] hover:text-primary-800 dark:hover:text-[#44bdff]/80 transition-colors uppercase tracking-tight flex items-center gap-3">
                  <FolderOpen className="w-8 h-8" /> Mis Proyectos
                </Link>
              )}
              {profile?.role === 'admin' && (

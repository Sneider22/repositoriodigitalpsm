...
import { Search, Filter, MapPin, BookOpen, Clock, FileText, ChevronDown, RotateCcw, Plus, ArrowUp, ArrowDown, ArrowUpDown } from 'lucide-react';
...
const SortButton = ({ label, sortKey, currentSort, currentDir, onClick }) => {
...
const Repository = () => {
  ...
  const sortedProjects = [...filteredProjects].sort((a, b) => {
    const getScore = (p) => calculateIRA({
      type: p.tipo, downloads: p.descargas, views: p.vistas,
      year: p.ano_publicacion || (p.created_at ? new Date(p.created_at).getFullYear() : new Date().getFullYear()),
      semester: p.semestre_id || p.semestre, career: p.carrera,
      title: p.titulo, summary: p.descripcion
    });
    ...
    if (sortBy === 'score') {
      ...
      const getTimestamp = (proj) => {
        // Si viene de Supabase tiene un string ISO válido
        if (proj.created_at) return new Date(proj.created_at).getTime();
        
        // Si es un Mockup, solo viene un año numérico puro (ej: 2024), 
        // lo forzamos a que sea el 1ero de Enero de ese año para poder compararlo.
        if (proj.ano_publicacion || proj.fecha_publicacion) {
          const yearObj = Number(proj.ano_publicacion || proj.fecha_publicacion);
          // Constructor Date(año, mes(0=Ene), dia)
          return new Date(yearObj, 0, 1).getTime(); 
        }
        return 0; // Fallback
      };
      ...
      <div className="max-w-7xl mx-auto px-4 mt-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:inline-block text-sm font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full shadow-inner border border-gray-200 dark:border-gray-700">
              {sortedProjects.length} evaluados
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
            ...
            <SortButton label="Puntaje" sortKey="score" currentSort={sortBy} currentDir={sortDir} onClick={handleSort} />
            <SortButton label="Fecha" sortKey="date" currentSort={sortBy} currentDir={sortDir} onClick={handleSort} />
            <SortButton label="Título" sortKey="title" currentSort={sortBy} currentDir={sortDir} onClick={handleSort} />
          </div>
        </div>
        ...
        {/* Pagination Controls */}
        {!loadingProjects && sortedProjects.length > itemsPerPage && (
          <div className="mt-12 mb-8 flex flex-col items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {[...Array(Math.ceil(sortedProjects.length / itemsPerPage))].map((_, i) => (
                  <button
                    key={i + 1}
                    onClick={() => {
                      setCurrentPage(i + 1);
              ...
              <button
                onClick={() => {
                  setCurrentPage(prev => Math.min(Math.ceil(sortedProjects.length / itemsPerPage), prev + 1));
                  window.scrollTo({ top: 600, behavior: 'smooth' });
                }}
